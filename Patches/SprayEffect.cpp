...
DWORD* GetParticleTableAddr()
{
    ...
    // TODO: V1.1 and DC search addresses
    DWORD *addr = (DWORD*)ReadSearchedAddresses(0x00570B80, 0x0, 0x0, ParticleTableAddrSearchBytes, sizeof(ParticleTableAddrSearchBytes), 0x8);
    if (addr == nullptr)
    {
...
DWORD* GetFrameCounterAddr()
{
    ...
    // TODO: V1.1 and DC search addresses
    DWORD *addr = (DWORD*)ReadSearchedAddresses(0x0044932B, 0x0, 0x0, FrameCounterAddrSearchBytes, sizeof(FrameCounterAddrSearchBytes), 0x6);
    if (addr == nullptr)
    {
...
void PatchLyingFigureSprayEffectSpeed()
{
    ...
    // TODO: V1.1 and DC search addresses
    const DWORD SpraySpeedFactorState0Addr = SearchAndGetAddresses(0x004A40CD, 0x0, 0x0, State0SearchBytes, sizeof(State0SearchBytes), 0x25);
    ...
    // TODO: V1.1 and DC search addresses
    const DWORD SpraySpeedFactorState1Addr = SearchAndGetAddresses(0x004A401A, 0x0, 0x0, State1SearchBytes, sizeof(State1SearchBytes), 0xF);
    ...
    // TODO: V1.1 and DC search addresses
    const DWORD SpraySpeedFactorState2Addr = SearchAndGetAddresses(0x004A3F15, 0x0, 0x0, State2SearchBytes, sizeof(State2SearchBytes), 0xF);
...
void PatchLyingFigureSprayEffectSpawnRate()
{
    ...
    // TODO: V1.1 and DC search addresses
    DWORD SprayAllocAddr = SearchAndGetAddresses(0x004A4A1D, 0x0, 0x0, SprayAllocAddrSearchBytes, sizeof(SprayAllocAddrSearchBytes), -0xD);
    if (!SprayAllocAddr)
    {
    ...
    // TODO: V1.1 and DC search addresses
    LyingFigureSprayHandlerAddr = (DWORD*)ReadSearchedAddresses(0x004A4A1D, 0x0, 0x0, SprayAllocAddrSearchBytes, sizeof(SprayAllocAddrSearchBytes), -0x9);
...
// Reduce the spawn rate of Hyper Spray particles by half
void PatchHyperSprayEffectSpeed()
{
    ...
    // TODO: V1.1 and DC search addresses
    const DWORD SpraySpeedFactorState0Addr = SearchAndGetAddresses(0x004A4579, 0x0, 0x0, State0SearchBytes, sizeof(State0SearchBytes), 0xC);
    if (!SpraySpeedFactorState0Addr)
    {
        Logging::Log() << __FUNCTION__ << " Error: failed to find memory address for state 0!";
        return;
    ...
    // TODO: V1.1 and DC search addresses
    const DWORD SpraySpeedFactorState1Addr = SearchAndGetAddresses(0x004A44EC, 0x0, 0x0, State1SearchBytes, sizeof(State1SearchBytes), 0x13);
    if (!SpraySpeedFactorState1Addr)
    {
        Logging::Log() << __FUNCTION__ << " Error: failed to find memory address for state 1!";
        return;
    ...
    // TODO: V1.1 and DC search addresses
    const DWORD SpraySpeedFactorState2Addr = SearchAndGetAddresses(0x004A442C, 0x0, 0x0, State2SearchBytes, sizeof(State2SearchBytes), 0xF);
    if (!SpraySpeedFactorState2Addr)
    {
        Logging::Log() << __FUNCTION__ << " Error: failed to find memory address for state 2!";
        return;
...
void PatchHyperSprayEffectSpawnRate()
{
    ...
    // TODO: V1.1 and DC search addresses
    DWORD SprayAllocAddr = SearchAndGetAddresses(0x004A509D, 0x0, 0x0, SprayAllocAddrSearchBytes, sizeof(SprayAllocAddrSearchBytes), -0xD);
    if (!SprayAllocAddr)
    {
        Logging::Log() << __FUNCTION__ << " Error: failed to find memory address for Lying Figure spray alloc handler!";
        return;
    ...
    // TODO: V1.1 and DC search addresses
    HyperSprayHandlerAddr = (DWORD*)ReadSearchedAddresses(0x004A509D, 0x0, 0x0, SprayAllocAddrSearchBytes, sizeof(SprayAllocAddrSearchBytes), -0x9);
